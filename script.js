...
function sendToGoogleSheet(primary, multi) {
  ...
  const params = new URLSearchParams({
    gender: gender || "",
    ageGroup: ageGroup || "",
    highGlucose: healthConditions.highGlucose ? "是" : "否",
    hypertension: healthConditions.hypertension ? "是" : "否",
    hyperlipidemia: healthConditions.hyperlipidemia ? "是" : "否",
  ...
  console.log("Google Sheet 已送出（GET image beacon）", {
    gender: gender || "",
    ageGroup: ageGroup || "",
    highGlucose: healthConditions.highGlucose ? "是" : "否",
    hypertension: healthConditions.hypertension ? "是" : "否",
...
let ageGroup='';
let healthConditions={highGlucose:false,hypertension:false,hyperlipidemia:false,none:false};
...
const partnerOf={
  shiRe:"小龍夥伴", xuRe:"月光貓夥伴", xuHan:"兔兔夥伴", tanShiRe:"浣熊夥伴",
  qiXu:"樹懶夥伴", xueXu:"獨角獸夥伴", qiYu:"小鹿夥伴", xueYu:"台灣藍鵲夥伴", pingHe:"烏龜夥伴"
};
...
function selectGender(g,el){gender=g;document.querySelectorAll('.choice,.healthChoice').forEach(c=>c.classList.remove('selected'));el.classList.add('selected');document.getElementById('genderNext').disabled=false}
function cleanQs(key){return stage2[key].qs.map(q=>typeof q==='string'?q:(q[gender]||null)).filter(Boolean)}
function toggleHealthCondition(key,el){
  if(key==='none'){
    healthConditions={highGlucose:false,hypertension:false,hyperlipidemia:false,none:!healthConditions.none};
  }else{
    healthConditions.none=false;
    healthConditions[key]=!healthConditions[key];
  }
  document.querySelectorAll('.healthChoice').forEach(c=>{
    const k=c.dataset.key;
    c.classList.toggle('selected',!!healthConditions[k]);
  });
}
function goHealth(){
...
function startStage1(){ ageGroup=document.getElementById('ageGroup')?document.getElementById('ageGroup').value:'';mode='stage1';current=0;answers=[];stage2Results=[];stage2Queue=[];allStage2Keys=[];renderQ();go('question')}
...
function finishStage1(){
 ...
 const highest=scores.filter(s=>s.score===maxScore);
 const qualified=maxScore>=6 ? highest : [];
 stage2Queue=qualified.length ? qualified.map(s=>s.key) : ['pingHe'];
 allStage2Keys=[...stage2Queue];

 const conclusion=qualified.length
   ? (qualified.length===1
      ? `第一階段最高分為 ${typeInfo[qualified[0].key].name}（${maxScore} 分）。<br>接下來將進入此體質的第二階段問卷。`
      : `第一階段有 ${qualified.length} 種體質並列最高分（${maxScore} 分）：${qualified.map(s=>typeInfo[s.key].name).join('、')}。<br>接下來將依序完成這些體質的第二階段問卷。`)
   : `第一階段最高分為 ${maxScore} 分，未達 6 分。<br>接下來將直接進入平和體質問卷。`;

 document.getElementById('stage1Conclusion').innerHTML=conclusion;
 document.getElementById('scoreTable').innerHTML=`<table class="scoreTable"><tr><th>體質</th><th>初步分數</th><th>是否進入第二階段</th></tr>${scores.map(s=>{
   const enter=qualified.some(q=>q.key===s.key);
   return `<tr><td>${typeInfo[s.key].name}</td><td>${s.score} 分</td><td>${enter?'是':'否'}</td></tr>`;
 }).join('')}</table>`;
...
function partnerCard(r,pose=2){
 const info=typeInfo[r.key];
 return `<div class="partnerCard">
   <img src="${safeImgFor(r.key,pose)}" alt="${partnerOf[r.key]}">
   <div class="partnerBody">
     <div class="partnerName">${partnerOf[r.key]}</div>
     <div class="partnerType">${info.name}｜${r.score}分｜${r.judge}</div>
     <p>${info.tip}</p>
     <ul>${info.advice.map(a=>`<li>${a}</li>`).join('')}</ul>
   </div>
 </div>`;
}
function showFinal(results,all){
 ...
 document.getElementById('finalAdvice').innerHTML=`<b>您的主要體質夥伴：${partnerOf[best.key]}</b><br><b>${typeInfo[best.key].tip}</b><ul>${typeInfo[best.key].advice.map(a=>`<li>${a}</li>`).join('')}</ul><div class="tiny">本結果為健康促進與衛教參考，非醫療診斷。</div>`;
 document.getElementById('multiResults').innerHTML=`<div class="partnerSectionTitle">您的體質夥伴</div>${results.length>1?`<div class="chips"><span class="chip">您可能有 ${results.length} 種體質傾向</span></div>`:''}${results.map((r,i)=>partnerCard(r,(i%4)+1)).join('')}`;
 document.getElementById('finalDetails').innerHTML=`<h2>完整分數</h2><table class="scoreTable"><tr><th>體質</th><th>分數</th><th>判定</th></tr>${all.map(r=>`<tr><td>${typeInfo[r.key].name}</td><td>${r.score}分</td><td>${r.judge}</td></tr>`).join('')}</table><p class="small">第二階段計算公式：[(總分－題數)/(題數×4)]×100。平和體質題目依量表特性採反向計分，判定門檻為 60 分以上。</p>`;
 sendToGoogleSheet(best.name||typeInfo[best.key].name, results.map(r=>typeInfo[r.key].name).join(', '));
 go('final');
}
function toggleDetails(){document.getElementById('finalDetails').classList.toggle('hidden')}
function restart(){hasSubmittedToSheet=false;gender=null;ageGroup='';healthConditions={highGlucose:false,hypertension:false,hyperlipidemia:false,none:false};mode='';current=0;answers=[];stage2Queue=[];stage2Results=[];document.querySelectorAll('.choice,.healthChoice').forEach(c=>c.classList.remove('selected'));document.getElementById('genderNext').disabled=true;go('home')}
