...
function finishStage1(){
 ...
 const conclusion=qualified.length
   ? `初步評估達標體質：${stage2Queue.map(k=>typeInfo[k].name).join('、')}<br>接下來將完成 ${stage2Queue.length} 份第二階段問卷。`
   : `目前八種偏性體質初步分數皆未達 5 分。<br>接下來將進入平和體質問卷。`;
 document.getElementById('stage1Conclusion').innerHTML=conclusion;
 document.getElementById('scoreTable').innerHTML=`<table class="scoreTable"><tr><th>體質</th><th>初步分數</th><th>是否進入第二階段</th></tr>${scores.map(s=>`<tr><td>${typeInfo[s.key].name}</td><td>${s.score} 分</td><td>${s.score>=5?'是':'否'}</td></tr>`).join('')}</table>`;
...
function restart(){hasSubmittedToSheet=false;gender=null;mode='';current=0;answers=[];stage2Queue=[];stage2Results=[];document.querySelectorAll('.choice').forEach(c=>c.classList.remove('selected'));document.getElementById('genderNext').disabled=true;go('home')}
